...
- **OC0x寄存器**应该在**设置端口引脚所对应的方向寄存器<sub>Data Direction Register</sub>之前设置**。最简单的方法是在普通模式Normal中，使用强制比较匹配，**FOC0x**，达到目的。在改变**波形发生模式<sub>Waveform Generator modes</sub>**时，**OC0x寄存器**的值不会改变。
- 
- 以下是一些可用的配置实例。
